...
public class PerformanceRepositoryImpl implements PerformanceRepositoryCustom {
  ...
  public List<GameSummary> findGameSummaries(String playerId, String playlist, Boolean isRanked) {
    String whereClause = getWhereClause(playerId, playlist, isRanked);
  ...
  private String getWhereClause(String playerId, String playlist, Boolean isRanked) {
    String whereClause = WHERE_CLAUSE.replace("{playerId}", playerId);
    if (playlist != null) {
      whereClause += ("and g.playlist = '" + playlist + "' ");
    }
    if (isRanked != null) {
      whereClause += ("and g.competitive_ind = " + (isRanked ? "1" : "0") + " ");
    }
    return whereClause;
  }
}
